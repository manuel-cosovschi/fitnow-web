  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, animation: 'fadeUp 0.4s ease-out' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16 }}>
        <div onClick={() => navigate('/run-navigator')} className="glass-card" style={{ padding: 24, textAlign: 'center', cursor: 'pointer', border: '1px solid var(--fn-blue)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div onClick={() => navigate('/run-preview')} className="glass-card" style={{ padding: 20, cursor: 'pointer' }}>
        ...
        <div onClick={() => navigate('/run-planner')} className="glass-card" style={{ padding: 20, cursor: 'pointer' }}>
          <History size={24} color="var(--fn-blue)" style={{ marginBottom: 12 }} />
          <h4 style={{ fontSize: 15, margin: '0 0 4px', fontWeight: 600 }}>Planificar</h4>
          <p style={{ fontSize: 12, color: 'var(--fn-slate)', margin: 0 }}>Crear ruta</p>
        </div>
      </div>
    </div>
  );
